...
namespace CM.FunctionalTests.Common
{
    ...
    public class SvnProviderTest
    {
        ...
        {
            Assert.That(!new SvnProvider(log).Exists(@"file:///missing/svn/repo"), log.Contents);
        }

        [Test]
        public void CreatedUrlShouldExist()
        {
            Using.SvnRepo(url => Assert.That(new SvnProvider(log).Exists(url), log.Contents));
        }

        [Test]
        public void ShouldNotExistIfEndpointDoesNotExist()
        {
            Using.SvnRepo(url => Assert.That(!new SvnProvider(log).Exists(url + "/test"), log.Contents));
        }
        ...
        public void ImportShouldAddFilesToRepository()
        {
            Using.SvnRepo(url =>
            {
                ...
                var provider = new SvnProvider(log);
